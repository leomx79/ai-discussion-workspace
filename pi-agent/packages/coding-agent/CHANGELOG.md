...
- Added Ansteel team state schema v12 with replayable attempted/applied/rejected lifecycle transitions, per-object `transitionLogId` bindings, runtime-trace mirroring, and fail-closed status, board, and doctor projection checks.
- Added a versioned Ansteel runtime event catalog that rejects unknown names and invalid event/outcome combinations on both write and read while preserving pre-catalog schema-v1 logs as read-only evidence. Run, role-session, provider-request, and tool-call spans now persist distinct lifecycle event names; resumed runs and public role-output metadata are explicit events, and every mechanical repair provider request has its own request ID and span.
- Added one controlled asynchronous runner for Ansteel task tests, milestone tests, and delivery checks. Runtime traces now record real process spawn, heartbeat, exit, and orphan-detection events with bounded output metadata and parent tool correlation.
- Added auditable runtime run-writer leases. Traces now record lock-backed acquire, renewal, expiry, and release facts under one lease ID; a per-run audit gate records release success only after the OS lock is removed and serializes the next owner, while crash recovery records the unconfirmed old lease as expired before acquiring a replacement.
- Added non-recursive artifact lifecycle evidence. Source events and new content-addressed artifact receipts share one durable log batch, deduplicated files are hashed before verification, and doctor or incident runs persist verified or missing results without attaching an artifact to its own lifecycle event.
- Added mechanical Security runtime evidence. Real role-tool policy refusals now derive `security.access-denied` from a failed tool terminal, while persistence-time secret detection derives count-only `security.secret-detected` and `security.redaction-applied` records in the source batch. Security records reject recursive artifacts and secret-bearing audit payloads before any artifact I/O.
- Added mechanical per-stage read-only tool budget evidence. Isolated role stages now record `budget.reserved`, each permitted read-only preflight records `budget.consumed`, and the first over-limit request records `budget.exhausted` with `budget-exhausted` before execution, retaining counters and tool-call identity without persisting raw tool arguments.
- Completed product write paths for all 37 protocol runtime events. Authoritative AgentSession retries now produce `provider.request.retry`, provider length stops produce failed `role.session.truncated` records, governed process heartbeats derive correlated `tool.call.progress`, and an incident run can durably record `event.chain.invalid` in an isolated hash-chained segment without rebuilding or replacing an untrusted runtime index.

### Fixed

- Allowed an Ansteel RPC host to recover a crash-left runtime or index lock immediately when its validated private owner record names a PID that definitively no longer exists. Live owners, missing or malformed metadata, permission ambiguity, PID reuse, and unexpected lock-directory contents remain fail-closed; the owner record stores hashes instead of raw command, working-directory, or credential values.
