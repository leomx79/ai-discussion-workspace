	...
	if (parsed.ansteel !== undefined || parsed.ansteelResume !== undefined) {
		...
		try {
			...
			const review = await runAnsteelProjectReview({
				...
				createRoleSession: async ({
					...
					const reviewToolPolicy = createAnsteelReviewToolPolicy(roleCwd, {
						allowBashComputation: config?.allowBashComputation === true,
					});
					...
					created.session.agent.beforeToolCall = async (context, signal) => {
						...
						const policyResult = reviewToolPolicy.beforeToolCall(context.toolCall.name, context.args);
						if (policyResult?.block) {
							toolBudget.recordBlockedToolCall(policyResult.reason);
							return policyResult;
						}
					...
					const rawTurnSession = createAnsteelRawTurnSession({
						prompt: async (text, promptOptions) => {
							...
							toolBudget.reset({ toolsEnabled: toolsEnabled && promptOptions?.formatRepair !== true });
						...
						reset: () => created.session.agent.reset(),
			...
			const reportPath = writeAnsteelReport({
				...
				topic: review.topic,
				markdown: `${review.markdown}\n## Role Models\n\nConfigured/resolved role identities:\n${roleModels}\n\n${modelBoundary}\n`,
			});
