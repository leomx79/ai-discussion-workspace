...
/ansteel-team verify TASK-MOTOR "C:\\ansteel-manifests\\motor-delivery.json"
/ansteel-team status
...
`nextAction.kind=report` is a mechanically green, no-side-effect public update or coordinator task-assignment request. A real project-direction `decision` remains yellow and requires the normal exact peer action reviews; reporting must not be mislabeled as a decision merely to fit the schema.

Before the coordinator creates revisioned work, a taskless yellow or red checkpoint remains visible as pending governance. After a task exists, a checkpoint without that exact `taskId` is public context only: it cannot authorize the task action and cannot keep an approved, independently delivered task pending. Current task governance considers only action bindings whose version matches the task's current revision.
...
State schema v12 keeps lifecycle transition history inside `team.json`. Every team, role, challenge, task, milestone, checkpoint, process issue, and delivery verification stores the `transitionLogId` of its latest applied transition. A lifecycle attempt appends an `attempted` record followed by either `applied` or `rejected`, including the guard result and stable failure reason where applicable; the same records are mirrored into the existing runtime trace. Loading state, rendering status or the shared board, and running doctor replay these records and fail with `state-projection-mismatch` when a status, transition record, or transition pointer diverges. Validation, authorization, and command-admission errors that occur before a lifecycle object or transition exists remain ordinary operation failures and are not represented as fictional transitions.
...
Every runtime command receives a `runId` and `traceId`; each operation receives a `spanId` and, when nested, a `parentSpanId`. Records can also carry `teamId`, `role`, `taskId`, `checkpointId`, `toolCallId`, provider/process identifiers, revision information, and a direct cause identifier. Failures use the versioned reason-code set, including `provider-timeout`, `provider-empty-public-output`, `provider-rate-limited`, `provider-authentication-failed`, `tool-exit-nonzero`, `tool-timeout`, `tool-policy-denied`, `process-orphaned`, `event-chain-invalid`, `event-fsync-failed`, `artifact-missing`, `state-projection-mismatch`, `budget-exhausted`, `secret-detected`, `no-governed-progress`, `coordinator-restarted`, and `unclassified-runtime-error`.

New runtime records also carry `eventCatalogVersion: 1`. The catalog mechanically restricts every persisted event name to its allowed outcome values on both write and read. An unknown catalog version, unknown name, or invalid combination fails with `event-chain-invalid`; schema-v1 records created before the catalog field existed remain readable as legacy evidence and are never silently rewritten. Run, role-session, provider-request, and tool-call spans use distinct lifecycle names rather than reusing one name with a different outcome. A mechanical governance repair creates a second provider request ID and span. An authoritative AgentSession `auto_retry_start` instead writes `provider.request.retry` under the currently active request, recording only attempt, maximum attempts, and delay; its terminal request record carries the resulting retry count. Provider records include the configured provider/model identity, request round, stage timeout, duration, output length, and empty-output status; unavailable first-token, token-count, and HTTP telemetry is stored as `null`, not inferred. An assistant stop with `stopReason: length` writes `role.session.truncated/failed` with `budget-exhausted` and cannot become a successful role report.

The same catalog version closes the complete v1 entry envelope. Required and optional top-level fields are explicit; unknown fields, non-canonical UTC timestamps, invalid monotonic nanoseconds, unsupported levels or roles, malformed run/trace/span/task/checkpoint/issue/tool/provider/process/lease identifiers, unsafe revisions, non-SHA-256 hashes, self-parented spans, and invalid artifact references fail before persistence. Artifact storage IDs must resolve to the run project's own `artifacts/<sha256>` namespace. One JSONL record is limited to 256 KiB, with an 8 KiB message, 128 KiB structured data, 12 nested levels, 128 array items or object properties, 64 KiB strings, and 16 artifact references. The writer preflights the complete input batch, including redacted values and predicted content-addressed references, before the first artifact write; the reader applies the same envelope and resource checks to `eventCatalogVersion: 1` before accepting the hash chain. Pre-catalog schema-v1 evidence remains on the intentionally weaker read-only compatibility path. Adding a v1 field requires a new catalog version rather than silently widening this contract.

Content-addressed runtime artifacts have their own non-recursive lifecycle records. The writer redacts and durably stores content before committing the source event, then writes the source event and `artifact.stored` in one runtime-log fsync batch. If the content-addressed file already exists, it is read and hashed before `artifact.verified` is allowed. `doctor` and `incident` also re-read every referenced file and append `artifact.verified` or `artifact.missing` to their own observed run, linked to the source run and sequence. Artifact lifecycle events always have an empty `artifactRefs` array, and a lifecycle event that attempts to attach another artifact is rejected before any artifact file is created. These facts do not implement retention or garbage collection; referenced artifact preservation remains mandatory.

Runtime Security records come only from mechanical execution boundaries. Before any runtime-log or artifact I/O, the writer scans the source message, nested data, and artifact content. When the safe value actually differs, the same durable log batch appends `security.secret-detected` and `security.redaction-applied` with the source event hash, fixed surface names, and counts only; it never stores the secret value, field path, or a secret hash. Existing `[REDACTED]` markers and telemetry fields such as `inputTokens`, `outputTokens`, `maxTokens`, and `tokenCountsAvailable` do not create false findings. A role tool rejected by the real `beforeToolCall` policy records a failed tool terminal and derives `security.access-denied` with the same role, tool-call identity, parent span, and source hash. Security lifecycle records cannot attach artifacts or contain a payload that itself needs redaction; the complete input batch is rejected before artifact creation if either invariant is violated. These records prove detection and enforcement happened, not that every possible credential form or authorization rule is already covered.

Each isolated role stage also records its mechanical read-only tool allowance. Stage reset writes `budget.reserved` with `used: 0`; every permitted `read`, `grep`, `find`, `ls`, or read-only `bash` preflight writes `budget.consumed` before execution; the first request beyond the configured limit writes `budget.exhausted` with reason `budget-exhausted` and is blocked before execution. Consumed and exhausted records retain the role span and `toolCallId`, but never copy the model-supplied path, pattern, or command arguments. `used`, `limit`, and `remaining` are coordinator counters. These events cover only the per-stage read-only allowance: mutation tools and Ansteel task tools do not consume it, and it is not evidence for project-wide tool, token, time, disk, or task/file lease budgets.
...
`status --explain` combines persistent team status with the most recent completed run diagnosis. `trace` reloads and validates matching runtime records from disk. Before `doctor` starts an observed command, it first validates the complete persisted public-event hash chain independently, then re-reads `team.json` and verifies its ledger cursor and replayed public collaboration projection. A damaged event chain fails with `event-chain-invalid`; valid events paired with malformed state, a changed ledger cursor, or a divergent public projection fail with `state-projection-mismatch`. This preflight deliberately happens before any doctor log writer is created, so damaged evidence cannot be repaired or masked by a new diagnostic run. `doctor` then verifies the selected run's log chain and artifact hashes. A syntactically valid run ID with no persisted logs fails with `artifact-missing`; every `started` span must have a later catalog-defined terminal event with the same `spanId` and `parentSpanId`, otherwise the run fails with `process-orphaned`. The command fails whenever persisted-team integrity or the selected runtime is unhealthy. Each runtime run has one persistent writer lease. A new host cannot finalize spans while the original logger still renews that lease, and a second recovery writer cannot reuse the same sequence/hash head. Runtime and index locks have a coordinator-private owner record containing the owner ID, PID, process start time, and hashes of the executable, command, and working directory. A restarted host removes a crash-left empty lock directory early only when that record is valid and the PID definitively no longer exists; a live or reused PID, missing or malformed metadata, permission ambiguity, or unknown lock-directory content remains locked. The public runtime trace records `lease.acquired` only after the OS grants the run lock and `lease.renewed` only after the lock directory is successfully refreshed. Release and acquisition use a short-lived per-run audit gate: `lease.released/succeeded` is appended only after the OS lock directory is actually removed and before a new owner can enter. A failed removal writes no release success. If a restarted host finds a previous owner without a durable release or expiry receipt, it appends `lease.expired/failed` under the previous `leaseId`, then `lease.acquired` under the replacement ID. These events cover the runtime run writer only; the internal index lock and task/file authorization leases are not represented as product lease events. After acquiring the released or safely recovered lease, recovery re-reads the chain and only then appends the catalog-defined `abandoned/process-orphaned` terminal in the original run, trace, team, parent-span, causal and operation correlation chain. The recovery attempt that appends those terminals fails closed; a later explicit retry may continue the existing role-recovery path. `incident` creates an always-redacted, content-addressed JSON manifest from existing facts; it does not ask a model to guess the cause. If its target runtime chain is damaged, an explicitly isolated incident writer records `event.chain.invalid` in its own hash-chained and fsynced segment. It never rebuilds or replaces the unverifiable shared index, so ordinary writers remain fail-closed until the source evidence is repaired or handled externally. Extension-command errors are rethrown after the visible timeline message, so RPC returns `success: false` instead of reporting a failed command as successful.
...
As of 2026-07-31, the default branch `main` of this repository is protected by GitHub ruleset `20106294` (`Ansteel external anchor protection`, enforcement `active`, rules: `deletion` + `non_fast_forward`, empty bypass list, current user cannot bypass), so the source-branch dimension of this operational prerequisite is satisfied and remains independently verifiable through the GitHub rulesets API. The Git notes ref dimension still relies on fail-closed verification, because GitHub rulesets cannot target non-branch refs such as `refs/notes/**`.

Redaction happens before hashing and writing. Runtime records and artifacts must not contain API keys, authorization headers, cookies, private keys, environment-variable secret values, raw provider request bodies, private provider payloads, or hidden chain-of-thought. Environment assignments are matched by the complete variable name and sensitive suffix, so both bare names such as `API_KEY`, `TOKEN`, `PASSWORD`, and `SECRET` and provider-prefixed names such as `OPENAI_API_KEY` and `ANSTEEL_TL_API_KEY`, including mixed case and quoted values, are redacted in messages, nested data, and artifacts. Credential field matching is suffix-anchored so ordinary token-count telemetry remains visible. Public role updates remain project data in the separate collaboration ledger. Logs keep only the structured result, correlation metadata, output length, reason code, and redacted evidence needed to diagnose the run.

本阶段已实现黄色/红色动作的动态执行前阻断。成功的自动恢复会写入公共审计事件；成功的历史运行索引重建会写入可由 `trace` 查询的协调器审计事实。只有恢复或索引重建成功才记录对应成功事实；活跃宿主、锁冲突、损坏日志链或其他失败不会伪造成功事件。持久团队重启和显式 `/task TASK-ID` 恢复都生成新的 `runId`；前者沿用最近一次非诊断工作运行的 `traceId`，后者按 `taskId` 沿用该任务自己的 trace，避免多任务串线。根 `run.started` 记录产品/扩展版本、Git commit、脱敏配置指纹、功能开关、Node/OS/架构和项目根稳定标识；环境变量只记录启用名称，不记录值。真实 RPC 宿主强杀探针已证明开放的根、角色和 provider span 可由重启宿主归档为 `abandoned/process-orphaned`，恢复尝试先失败关闭，随后显式重试沿同一 trace 继续持久团队。这些门禁证明的是动作绑定、确认顺序和可追溯执行边界，不是最终交付正确性证明；持续协作后的最终双独立验收仍然保留。真实三提供商 r28-r30 已在冻结单任务夹具上 3/3 达到四轴终态，r34 已完成双任务失败 delivery 恢复，但长任务、存活/PID 复用进程的安全重接管和任意模型组合仍未证明。

任务测试、里程碑集成测试和 coordinator delivery 检查统一使用受控异步进程 runner。运行日志在 OS 返回真实 PID 后写 `process.spawned`，长运行写 `process.heartbeat`，退出后写 `process.exited`；当该进程由角色工具 span 触发时，每个 heartbeat 还在同一 durable batch 派生 `tool.call.progress`，共享 `toolCallId`、`processId` 和工具父子 span。记录命令和工作目录哈希、允许策略、PID、超时、退出码、信号、持续时间、最后进展时间、输出字节数、截断状态和完整 stdout/stderr 流哈希。输出超过采集上限时继续排空管道，但以 `budget-exhausted` 失败关闭，不把截断证据当成成功。协调器恢复开放的进程 span 时先写 `process.orphan-detected`，再归档 `process.exited/abandoned`。这能证明检测和失败关闭，不表示仍存活或 PID 已复用的进程可以安全重接管。
...
状态 v11 已接入受信任、可重放的任务交付验证记录。任务或里程碑批准、黄色/红色动作会签、Git 提交以及 GitHub Actions 的文本仍不能把 `deliveryStatus` 升级为 `passed`；只有协调器执行 `/ansteel-team verify`，并由签名事件链重放出当前 task revision、冻结 diff、整仓工作区、Git HEAD、manifest 和全部检查均匹配的成功收据，`workflowStatus` 才成为 `completed`。失败检查、超时、启动失败、diff/workspace 漂移、HEAD 漂移或中断均失败关闭。

`verify` 的 manifest 必须位于项目目录外，使用 version 1、精确 task ID/revision 和结构化 `executable`/`args`/`timeoutMs`。命令以 `shell: false` 和环境白名单运行，不继承 API key 或 token；完整 stdout/stderr 写入 manifest 同目录下的项目外内容寻址证据目录，团队状态和公共账本只保存哈希、退出结果和稳定原因码。`.pi` 协调器审计文件不属于产品工作区绑定，其他 tracked/untracked 文件都参与漂移检测。
...
2. A task with unmet predecessors is coordinator-derived as `blocked`; no role can write its files or assert that it has been unlocked. It becomes `claimed` only after every predecessor is governance-approved **and** has a current-revision passed delivery receipt. Only then can its owner use `edit` or `write` on those exact files. All other paths, all other roles, and a submitted task are blocked. Direct `bash` is inspection-only; it cannot be used to bypass the file gate.
...
6. Both final peer approvals mark the governance task state `approved`, but do not unlock dependents or claim final delivery. Any final rejection returns it to `revision-required`; the owner must change it, run a new test, and submit a new diff. QA has the same immediate return authority and cannot be bypassed.
7. The coordinator invokes `/ansteel-team verify <TASK-ID> <absolute-manifest-path>`. Only a successful current-revision delivery receipt unlocks dependents and milestones. A failed check records `task-delivery-failed`, keeps governance history, moves the task to `revision-required`, and requires a new owner revision before delivery can pass again.

`/ansteel-team task TASK-ID` resumes an existing non-approved task without creating a duplicate. A coordinator task runs through isolated owner epochs. `teamTaskMaxEpochs` controls the maximum owner epochs (`1..128`, default `8`), while `teamTaskMaxNoProgressEpochs` controls consecutive epochs without governed progress (`1..8`, default `2`, and never greater than the maximum epoch count). Delivery progress means a change to task status or revision, recorded test/submission/collaboration-update/review counts, or the SHA-256 of the exact claimed-file Git diff. Reading files, repeating tools, producing prose, or extending wall-clock time is not delivery progress.

Pre-submission collaboration is counted separately from delivery. The coordinator reconstructs a revision-bound collaboration fingerprint from durable task checkpoints, process issues, process resolutions and their reviews, and peer action reviews. One `/ansteel-team task` command may use at most one new collaboration fingerprint to reset the consecutive no-progress counter. Further collaboration-only epochs count as no delivery progress, and `teamTaskMaxEpochs` always remains the hard ceiling. Collaboration never changes task revision or delivery status and never creates test, submission, collaboration-update, or final-review evidence. Because the fingerprint is derived from persisted state rather than model prose, a restart reconstructs the same value.
