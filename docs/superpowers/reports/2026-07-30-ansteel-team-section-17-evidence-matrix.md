...
| 17.3 统一关联标识 | 已实现基础，真实跨进程探针通过 | 日志 schema、索引和查询覆盖 run、trace、角色、任务、检查点、问题、工具、provider、进程、租约、revision、diff 和 cause ID。持久团队重启和显式 `/task TASK-ID` 恢复均生成新 `runId` 并记录 `resumedFromRunId/resumedFromSequence`。真实 RPC 探针在 TL provider 请求中强制终止第一个宿主；第二个宿主发现 1 个孤儿运行，先以原 `leaseId` 写 `lease.expired`，再以新 `leaseId` 获取恢复写权，把根、角色和 provider 共 3 个开放 span 归档为目录定义的 `run.failed`、`role.session.ended`、`provider.request.completed` 与 `abandoned/process-orphaned`，成功释放恢复 lease；该恢复命令失败关闭，下一次显式 `start` 和后续 `ask` 沿原 `traceId` 成功恢复同一团队。artifact 生命周期事件通过 `causeEventId` 或 source run/sequence 绑定；Security 事件以源事件 hash 作为 `causeEventId`；Budget 与工具进展保留角色 `parentSpanId`、`toolCallId` 和 `processId`，不保存工具参数。环境变量仅记录启用名称，测试密钥值、Bearer 与 `sk-` 形态未进入 trace。当前定向回归为可观测性 56/56、核心与进程 runner 118/118、扩展 55/55、CLI/RPC 18/18，共 247/247。 | 已证明原宿主明确退出后的 trace 与 run lease 连续性，以及内容 artifact、Budget、Security、retry、truncation、tool progress 和 chain-invalid 的基础因果；没有证明存活或 PID 复用进程的安全重接管，也没有覆盖日志截断和轮转恢复。配置敏感字段先替换为固定脱敏值再计算哈希。 | 将真实宿主、artifact、Budget、Security 和四个补齐事件的 RPC 探针保留为回归门禁；在 17.8 中继续补存活/PID 复用进程身份验证、截断和轮转故障注入。 |
| 17.4 结构化日志格式 | 部分实现，每事件 data schema 校验已完成 | 版本化单行 JSON、单调时钟、序号、哈希链、结构化 data、artifact 引用和稳定原因码已落盘。`ANSTEEL_RUNTIME_EVENT_DATA_SCHEMAS` 与事件目录逐键对应，显式定义每个事件的必填/可选字段、类型、未知字段拒绝策略和正常/失败/恢复 union；安全整数、非负数、SHA-256、规范 UTC 时间、嵌套环境指纹及 provider、process、lease、artifact、Budget、Security、状态转换的跨字段事实均机械校验。writer 在任何 artifact I/O 前失败关闭，reader 对 `eventCatalogVersion: 1` 同样拒绝非法 data；无目录版本的旧 schema-v1 记录继续只读兼容。变形测试覆盖缺字段、错类型、非法边界、未知字段和非法跨字段组合，并逐一写读全部合法 `eventName × outcome`。 | entry 外层通用字段仍只严格检查部分必填项，尚未完整限制 `timestampUtc`、单调时钟、level、message、artifactRefs 及全部可选关联 ID 的格式和整体大小/深度；因此不能把 17.4 整体标为完成。v1 data 未知字段明确失败关闭，未来扩展字段必须升级目录版本，不能静默放行。 | 下一步补齐版本化 entry envelope、关联 ID/时间/哈希格式、artifactRefs 结构和日志记录大小/深度上限，再用重哈希 reader 变形夹具与真实 CLI/RPC 回归证明。 |
| 17.5 必须记录的运行事件 | 已实现基础，目录与产品写路径均 37/37 | 目录 v1 定义全部 37 个协议事件和当前稳定内部事件，真实产品写路径也精确产生 37 个。新增事实来自真实 AgentSession retry、`stopReason: length`、工具绑定进程 heartbeat 和损坏链 incident；它们分别写 provider retry、role truncated、tool progress 和 chain invalid。incident 的隔离诊断段保持自身哈希链与 fsync，但不修复或替换损坏时不可验证的索引，普通 writer 继续失败关闭。artifact、Security、Budget、lease 与 process 的既有真实性边界保持不变。真实 RPC 覆盖正常任务、31 秒 task heartbeat/progress、provider retry、角色截断、损坏链 incident、5-read 预算耗尽、宿主强杀、artifact 存储/校验、无日志 run 缺失和角色越权读取。当前四组回归共 247/247 通过。 | 37/37 只证明协议事件各有受约束的产品产生路径。Budget 仍只覆盖隔离 stage 的只读工具调用数；runtime lease 不含内部 index 锁、任务/文件授权租约或动态转交；进程事件未直接携带全部执行前后文件哈希；provider token/首 token/HTTP 状态仍需底层权威回调；日志 entry 外层完整 schema、轮转与保留未完成。孤儿检测不等于存活/PID 复用进程已安全重接管。 | 保留每个事件的 writer/reader 和真实 CLI/RPC 回归；下一步扩展预算类型、任务/文件租约、entry envelope、轮转保留与崩溃故障注入。 |
...
4. 17.4 的每事件 data schema 已覆盖目录全部合法组合并在 writer/reader 双边失败关闭；17.5 的 process、runtime run lease、artifact、Budget、Security、role truncated、provider retry、tool progress 与 chain invalid 已完成权威产品接线，下一步补 entry envelope、预算类型和任务/文件租约。
