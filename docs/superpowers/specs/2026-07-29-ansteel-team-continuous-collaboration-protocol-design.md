...
| 9 | 接入 `collaborationStatus`、`governanceStatus`、`deliveryStatus` 三轴状态。 | 已实现，待远端验收 | 以只读持久事实投影三轴和 `workflowStatus`，`status --explain` 与工作板共享事件重放门禁。任务批准、动作确认或治理通过均不能推导交付通过；当前没有受信任、可重放的交付证据，因此 `deliveryStatus` 保持 `not-started`。本地构建、类型检查和六文件串行回归 `325 passed` 已通过，待本提交的 GitHub Actions 验收。 |
