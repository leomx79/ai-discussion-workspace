...
## 2026-08-01 补充：交付进展与协作进展双计数

r13-r18 的真实三提供商运行证明，严格只承认交付指纹会把已经形成 checkpoint、流程问题、
resolution/review 和 peer action review 的深度协作误判为完全无进展。修订后的协调器保留原交付指纹，
同时从持久化状态机械计算当前任务 revision 的协作指纹。

协作指纹只包含与当前 revision 绑定的任务 checkpoint，以及这些 checkpoint 上新增的流程问题、流程解决、
解决复核和 peer action review。`role-report`、自然语言复述、只读调用、`action-assessed` 和重复 ID 均不计入。
它不修改任务 revision、交付状态，也不生成测试、提交、提交后协作更新或最终评审证据。

每次 `/ansteel-team task` 调度最多消费一次协作续跑：当交付指纹未变化、协作指纹变化且额度尚未使用时，
协调器把连续无进展计数清零一次；额度耗尽后，继续产生协作事实也按无交付进展累计。
`teamTaskMaxEpochs` 始终是不可突破的硬上限。协作指纹完全由已持久化状态重建，因此进程重启不会依赖模型文本
或内存中的临时声明来判断进展。
